		    ...
		    struct sockaddr_storage *sock)
{
	...
	printf("is_in_addr_set: set:%s tmp:%s\n",
		cidr_to_str(set->subnet, CIDR_IPV4|CIDR_VERBOSE),
		cidr_to_str(cidr_tmp, CIDR_IPV4|CIDR_VERBOSE));
...
int cidr_to_addr_set(CIDR *cidr, struct ip_addr_set **set)
{
	...
	printf("cidr_to_addr_set1: my_set:%s arg:%s\n",
		cidr_to_str(my_set->subnet, CIDR_IPV4|CIDR_VERBOSE),
		cidr_to_str(cidr, CIDR_IPV4|CIDR_VERBOSE));

	/* we need to build a CIDR that include the old and the new cidr
	 * the old one is extended until it includes the new one */
	while (cidr_contains(my_set->subnet, cidr) != 0) {
		CIDR *super;

		super = cidr_net_supernet(my_set->subnet);
		if (super == NULL)
			return -ENOMEM; /** @todo more cleanups ? */

		cidr_free(my_set->subnet);
		my_set->subnet = super;
	}

	printf("cidr_to_addr_set2: my_set:%s arg:%s\n",
		cidr_to_str(my_set->subnet, CIDR_IPV4|CIDR_VERBOSE),
		cidr_to_str(cidr, CIDR_IPV4|CIDR_VERBOSE));
	...
	if (cidr_contains(my_set->subnet, cidr) == 0) {
		...
		printf("cidr_to_addr_set: my_set:%s mask=%u\n",
			cidr_to_str(my_set->subnet, CIDR_IPV4|CIDR_VERBOSE),
			my_set->bitmap[0]);
		return 0;
	}
		      ...
		      struct ip_addr_set **set)
{
	...
	errno = 0;
	if (sock->ss_family == AF_INET) {
		sk = &((struct sockaddr_in *)sock)->sin_addr;
		cidr_tmp = cidr_from_inaddr(sk);
	} else if (sock->ss_family == AF_INET6) {
		sk6 = &((struct sockaddr_in6 *)sock)->sin6_addr;
		cidr_tmp = cidr_from_in6addr(sk6);
	} else {
		LogCrit(COMPONENT_CONFIG,
			 "This address is neither v4 or v6");
			...
			struct ip_addr_set **set)
{
	...
	return addr_to_addr_set((struct sockaddr_storage *)result->ai_addr,
				 set);
}

/**
 * @brief Callback function, to be used with nl_map or nl_map_condensed
 *
 * This function is a callback function to be used with the map calls
 * inside the Nodelist part of the code. The nodelist map call will invoke
 * the callback each time it find a new node while expanding a nodelist
 *
 */
