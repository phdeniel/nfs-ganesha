...
static struct nullfs_fsal_obj_handle *alloc_handle(int dirfd,
                                                nullfs_file_handle_t *fh,
                                                struct stat *stat,
                                                nullfs_file_handle_t *dir_fh,
                                                const char *path,
                                                struct fsal_export *exp_hdl)
{
	...
	hdl = gsh_calloc(1, (sizeof(struct nullfs_fsal_obj_handle) +
			     sizeof(nullfs_file_handle_t)));
	if(hdl == NULL)
		return NULL;
	hdl->handle = (nullfs_file_handle_t *)&hdl[1];
	memcpy(hdl->handle, fh, sizeof(nullfs_file_handle_t));
	...
	if(hdl->obj_handle.type == REGULAR_FILE) {
		...
		if(link_content == NULL)
			goto spcerr;
                ...
                retlink = nullfs_readlink_by_handle(fh, dirfd, path,
                                link_content, len);
	...
	} else if(nullfs_unopenable_type(hdl->obj_handle.type)) {
                /* AF_UNIX sockets, character special, and block
                   special files  require craziness */
                if(dir_fh == NULL) {
                        int retval;
                        nullfs_alloc_handle(dir_fh);
                        retval = nullfs_fsal_fd_to_handle(exp_hdl, dirfd, dir_fh);
                        if(retval < 0) {
                                goto spcerr;
                        }
                }
		hdl->u.unopenable.dir = gsh_malloc(sizeof(nullfs_file_handle_t));
