...
{
	struct clovis_fsal_export *myself;

	myself = container_of(exp_hdl, struct clovis_fsal_export, export);
	return myself->p_vfs;
}
			...
			fsal_fsinfo_options_t option)
{
	...
	info = clovis_staticinfo(exp_hdl->fsal);
...
static fsal_status_t clovis_extract_handle(struct fsal_export *exp_hdl,
					 fsal_digesttype_t in_type,
					 struct gsh_buffdesc *fh_desc,
					 int flags)
{
	struct clovis_file_handle *hdl;
	size_t fh_size;
	...
	hdl = (struct clovis_file_handle *)fh_desc->addr;
	fh_size = clovis_sizeof_handle(hdl);
...
/* clovis_export_ops_init
 * overwrite vector entries with the methods that we support
 */

void clovis_export_ops_init(struct export_ops *ops)
{
	ops->release = release;
	ops->lookup_path = clovis_lookup_path;
	ops->extract_handle = clovis_extract_handle;
	ops->create_handle = clovis_create_handle;
...
{
	struct clovis_fsal_export *myself = NULL;
	int retval = 0;
	fsal_errors_t fsal_error = ERR_FSAL_INVAL;
	libzfswrap_vfs_t *p_zfs = NULL;

	myself = gsh_calloc(1, sizeof(struct clovis_fsal_export));

	fsal_export_init(&myself->export);
	clovis_export_ops_init(&myself->export.exp_ops);
