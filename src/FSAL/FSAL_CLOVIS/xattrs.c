				  ...
				  unsigned int *p_nb_returned, int *end_of_list)
{
	...
	struct clovis_fsal_obj_handle *obj_handle = NULL;
	...
	obj_handle =
	    container_of(obj_hdl, struct clovis_fsal_obj_handle, obj_handle);
	...
	retval = libzfswrap_listxattr(ZFSFSAL_GetVFS(obj_handle->handle),
				      &cred,
				      obj_handle->handle->clovis_handle,
				      (char **)&names, &namesize);
	if (retval)
	...
	for (index = 0; index < XATTR_COUNT; index++) {
		if (do_match_type(xattr_list[index].flags,
		    ...
		    && !strcmp(xattr_list[index].xattr_name,
			       xattr_name)) {
			return clovis_getextattr_value_by_id(obj_hdl,
							   index, buffer_addr,
							   buffer_size,
							   p_output_size);
		}
				    ...
				    size_t buffer_size, int create)
{
	...
	rc = libzfswrap_setxattr(ZFSFSAL_GetVFS(obj_handle->handle), &cred,
				 obj_handle->handle->clovis_handle, xattr_name,
				 buffer_addr);
					  ...
					  size_t buffer_size)
{
	...
	retval =
	    xattr_id_to_name(ZFSFSAL_GetVFS(obj_handle->handle), &cred,
			     obj_handle->handle->clovis_handle, xattr_id, name);
	if (retval)
		return fsalstat(retval, 0);

	return clovis_setextattr_value(obj_hdl, name, buffer_addr,
				     buffer_size, false);
}
