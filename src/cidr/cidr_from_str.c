...
CIDR *cidr_from_str(const char *addr)
{
	...
	if (buf != NULL || strcasecmp(addr+alen-5, ".arpa") == 0) {
		...
		if (buf == NULL) { /* If not set by .ip6.int above */
			...
			if (strncasecmp(addr+alen-9, ".ip6", 3) == 0) {
				toret->proto = CIDR_IPV6;
				buf = addr+alen-9;
			} else if (strncasecmp(addr+alen-13,
					       ".in-addr", 7) == 0) {
		 ...
		 * result.  This way, the calling program can tell the
		 * difference between "3.2.1..." and "0.3.2.1..." if it really
		 * cares to.
		 */
		...
		if (toret->proto == CIDR_IPV4) {
			for (i = 11; i <= 14;) {
				...
				/* Step backward until we at the start of
				 *  an octet */
				while (isdigit(*buf) && buf >= addr)
					buf--;

				/*
				 * Save that number (++i here to show that this
				 * octet is now set.
				 */
				...
				/*
				 * Back up a step to get before the '.', and
				 * process the next [previous] octet.
				 * If we were at the beginning of the string
				 * already, the test at the top of the loop
				 * will drop us out.
				 */
				buf--;
			}
			 ...
			 * describe how much information we've actually gotten,
			 * if we didn't get all 4 octets.  Because of the way
			 * .in-addr.arpa works, the mask can only fall on an
			 * octet boundary, so we don't need too many fancy
			 * tricks.  'i' is still set from the above loop to
			 * whatever the last octet we filled in is, so we don't
			 * even have to special case anything.
			 */
			for (j = 0; j <= i; j++)
				toret->mask[j] = 0xff;
			...
			/*
			 * This processing happens somewhat similarly to
			 * IPV4 above, the format is simplier, and we need to be
			 * a little sneakier about the mask, since it can fall
			 * on a half-octet boundary with .ip6.arpa format.
			 */
			for (i = 0; i <= 15; i++) {
				/* If we're before the beginning, we're done */
				if (buf < addr)
				 ...
				 * we're thru */
				if (buf == addr) {
					/* Shift back to skip error condition at
					 *  end of loop */
					buf--;
					break;
				}

				/* If we're not, stepping back should give
				 *  us a period */
				if (*--buf != '.') {
					/* Bad input */
					cidr_free(toret);
				...
				/*
				 * Step back and loop back around.
				 * If that last step back moves us to before the
				 * beginning of the string, the condition at the
				 * top of the loop will drop us out.
				 */
				while (*--buf == '.' && buf >= addr)
					/* nothing */;
			}
	...
	if (foundpf == 0) {
		...
		if (foundmask == 0) {
			/*
			 * If we didn't find a netmask, it may be that it's one
			 * of the v4 forms without dots.  Technically, it COULD
			 * be expressed as a single (32-bit) number that happens
			 * to be between 0 and 32 inclusive, so there's no way
			 * to be ABSOLUTELY sure when we have a prefix length
			 * and not a netmask.  But, that would be a
			 * non-contiguous netmask, which we don't attempt to
			 * support, so we can probably safely ignore that case.
			 * So try a few things...
			 */
			...
			if (pfx[1] == '0' && tolower(pfx[2]) == 'x')
				...
				/*
				 * If we get here, it looks like a decimal
				 * number, and we know there aren't any periods
				 * or colons in it, so if it's valid, it can
				 * ONLY be a single 32-bit decimal spanning the
				 * whole 4-byte v4 address range.  If that's
				 * true, it's GOTTA be a valid number, it's
				 * GOTTA reach to the end of the strong, and
				 * it's GOTTA be at least 2**31 and less
				 * than 2**32.
				 */
	...
	if (toret->proto == CIDR_IPV4) {
		...
		/*
		 * Handle the prefix/netmask.  If it's not set at all, slam it
		 * to the maximum, and put us at the end of the string to start
		 * out.
		...
		if (pflen > 0) {
			...
			/*
			 * Now pflen is in the 0...32 range and thus good.
			 * Set it in the structure.  Note that memset zero'd
			 * the whole thing to start.  We ignore mask[<12] with
			 * v4 addresses normally, but they're already set to
			 * all-1 anyway, since if we ever DO care about them,
			 * that's the most appropriate thing for them to be.
			 *
			 * This is a horribly grody set of macros.  I'm only
			 * using them here to test them out before using them in
			 * the v6 section, where I'll need them more due to the
			 * sheer number of clauses I'll have to get written.
			 * Here's the straight code I had written that the macro
			 * should be writing for me now:
			 *
			 * if (pflen>24)
			 *   for (j=24 ; j<pflen ; j++)
...
#undef UMIN
		...
		for (; i >= 0; i--) {
			/*
			 * As long as it's still a number or an 'x'
			 * (as in '0x'), keep backing up.  Could be hex,
			 * so don't just use isdigit().
			 */
			...
			/* If buf isn't pointing at one of [./'\0'],
			 *  it's screwed */
			if (!(*buf2 == '.' || *buf2 == '/'
			      || *buf2 == '\0')) {
				cidr_free(toret);
			...
			/*
			 * Now, because of the way compressed IPv4 addresses
			 * work, this number CAN be greater than 255, IF it's
			 * the last bit in the address (the first bit we parse),
			 * in which case it must be no bigger than needed to
			 * fill the unaccounted-for 'slots' in the address.
			 */
			if ((nocts != 0 && octet > 255)
			    || (nocts == 0 && octet >
				(0xffffffff >> (8*(nsect-1))))) {
				cidr_free(toret);
				errno = EINVAL;
				return NULL;
			}
			...
			/*
			 * If this is the 'last' piece of the address (the first
			 * we process), and there are fewer than 4 pieces total,
			 * we need to extend it out into additional fields.
			 */
			if (nocts == 1) {
				if (nsect <= 3)
					toret->addr[15-nocts++] =
			...
			/*
			 * If we've got 4 of 'em, we're actually done.  We got
			 * the prefix above, so just return direct from here.
			 */
			if (nocts == 4)
				return toret;
		}
		 ...
		 * parse backward.  However, to handle compressed form, if we
		 * hit a ::, we drop off and start parsing from the beginning,
		...
		/*
		 * If we got a netmask, rather than a prefix length, parse it
		 * and count the bits, like we did for v4.
		 */
		if (foundpf == 1 && foundmask == 1 && pflen == -1) {
			ctmp = cidr_from_str(pfx+1);
			if (ctmp == NULL) {
		...
		if (pflen > 0) {
			...
			/*
			 * Now save the pflen.  See comments on the similar
			 * code up in the v4 section about the macros.
			 */
#define UMIN(x, y) ((x) < (y) ? (x) : (y))
...
#undef UMIN
		...
		for (; i >= 0; i--) {
			/*
			 * First, check the . cases, and handle them all in
			 * one place.  These can only happen at the beginning,
			 * when we have no octets yet, and if it happens at all,
			 * we need to have 4 of them.
			 */
			if (nocts == 0 && addr[i] == '.') {
				i++; /* Shift back to after the '.' */

				for (; i > 0 && nocts < 4; i--) {
					/* This shouldn't happen except
					 *  at the end */
					if (addr[i] == ':' && nocts < 3) {
						cidr_free(toret);
						errno = EINVAL;
					...
					/* Should be a [decimal] octet right
					 *  after here */
				...
				/*
				 * At this point, 4 dotted-decimal octets
				 * should be consumed.  i has gone back one
				 * step past the : before the decimal, so
				 * addr[i+1] should be the ':' that preceeds
				 * them.
				 * Verify.
				 */
				if (nocts != 4 || addr[i+1] != ':') {
			 ...
			 * dotted-decimal stuff, or we've filled in nothing
			 * and have no dotted decimal.
			 */
			...
			if (addr[i] == ':' && addr[i+1] == ':') {
				/*
				 * If i is 0, we're already at the beginning of
				 * the string, so we can just return; we've
				 * already filled in everything but the
				 * leading 0's, which are already
		...
		if (i >= 0 && addr[i] == ':' && addr[i+1] == ':') {
			...
			/* Going this way, we do things
			 * a little differently */
			i = 0;
			while (i < j) {
				/*
				 * The first char better be a number.
				 * If it's not, bail (a leading '::' was
				...
				/*
				 * We should be pointing at the beginning of
				 * a digit string now.
				 * Translate it into an octet.
				 */
				...
				/*
				 * Discussion: If we're in this code block,
				 * it's because we hit a ::-compression while
				 * parsing from the end backward.  So, if we
				 * hit 15 octets here, it's an error, because
				 * with the at-least-2 that were minimized,
				 * that makes 17 total, which is too many.
				 * So, error out.
				 */
				if (nocts == 15) {
					cidr_free(toret);
				...
				/*
				 * If i==j, we're back where we started.
				 * So we've filled in all the leading stuff,
				 * and the struct is ready to return.
				 */
				if (i == j)
					return toret;
